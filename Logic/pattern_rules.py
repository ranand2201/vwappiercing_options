...
def piercing_direction(row):
    """Returns 'BUY', 'SELL', or None for a candle row (needs OPEN/CLOSE/VWAP)."""
...
def is_vwap_reentry_triggered(check_price, vwap, direction):
    ...
    VWAP in the original piercing direction (BUY: back above VWAP; SELL: back below VWAP).
    check_price is a candle's Close for the backtest/dry-run (candle-driven), or the live LTP for
    the tick-driven live engine -- same predicate either way.
    """
    return (check_price > vwap) if direction == "BUY" else (check_price < vwap)
